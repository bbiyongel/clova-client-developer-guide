...
<pre><code>{{ book.ServiceEnv.AuthAPIBaseURI }}</code></pre>
...
| `expires_in`    | number  | CLOVA access token의 유효 기간. 초 단위이며, `0` 이상의 정수 값을 가집니다.)    | 항상      |
...
| `expires_in`    | number  | 삭제한 CLOVA access token이 가졌던 유효 기간. 초 단위이며, `0` 이상의 정수 값을 가집니다.              | 항상      |
