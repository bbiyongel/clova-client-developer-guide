...
|------------------|-----------|-------------------------------------------|
| [`ExpectReport`](#ExpectReport) | Directive | 클라이언트에 현재의 설정 정보를 보고하도록 지시합니다. 클라이언트는 이 지시 메시지를 받으면 [`Settings.Report`](#Report) 이벤트 메시지를 CIC로 전송해야 합니다. |
...
| [`Update`](#Update)             | Directive | 클라이언트에 `payload`에 저장된 값을 설정값으로 적용하도록 지시합니다.  |
...
클라이언트에 현재의 설정 정보를 보고하도록 지시합니다. 클라이언트는 이 지시 메시지를 받으면 [`Settings.Report`](#Report) 이벤트 메시지를 CIC로 전송해야 합니다.
...
클라이언트에 `payload`에 저장된 값을 설정값으로 적용하도록 지시합니다.
