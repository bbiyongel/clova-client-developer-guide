...
| [`SynchronizeState`](#SynchronizeState)                | Directive | 클라이언트에 `payload`에 있는 데이터를 동기화하도록 지시합니다.            |
...
클라이언트에 `payload`에 있는 데이터를 동기화하도록 지시합니다. 클라이언트는 CIC로부터 전달된 데이터에 맞게 클라이언트에 설정된 값을 변경해야 합니다.
